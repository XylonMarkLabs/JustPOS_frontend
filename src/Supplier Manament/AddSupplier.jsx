...
const SUPPLIER_ID_PREFIX = "SUP_";
const SUPPLIER_ID_DIGITS = 4;

const generateNextSupplierId = (suppliers = []) => {
  const pattern = new RegExp(`^${SUPPLIER_ID_PREFIX}(\\d+)$`);

  const maxNumber = suppliers.reduce((max, supplier) => {
    const match = pattern.exec(supplier?.supplierId || "");
    if (!match) return max;
  ...
  return `${SUPPLIER_ID_PREFIX}${String(nextNumber).padStart(
    SUPPLIER_ID_DIGITS,
    "0"
  )}`;
};
...
const AddSupplierModal = ({ open, onClose, onAddSupplier, suppliers = [] }) => {
  ...
  useEffect(() => {
    if (open) {
      setFormData((prev) => ({
        ...prev,
        supplierId: generateNextSupplierId(suppliers),
      }));
    }
  }, [open, suppliers]);
  ...
  const handleSubmit = () => {
    ...
    if (!PHONE_REGEX.test(formData.contactNo)) {
      showError("Please enter a valid contact number", "Invalid Contact Number");
      return;
    }
    ...
    showSuccess(
      `Supplier "${formData.supplierName}" has been added successfully!`,
      "Supplier Added",
    );
    handleClose();
  };
